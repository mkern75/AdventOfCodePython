...
def topological_sort(wires, edges):
    ...
    return res if len(res) == len(wires) else None


wires = list(inputs.keys()) + list(gates.keys())
sorted_wires = topological_sort(wires, edges)
z_wires = sorted((wire for wire in wires if wire.startswith("z")), reverse=True)


def simulate(inputs, gates, wires_sorted):
    outputs = {}
...
outputs = simulate(inputs, gates, sorted_wires)
...
def find_rule(wire1, op, wire2):
    for k, (a, b, c) in gates.items():
        if (wire1, op, wire2) in [(a, b, c), (c, b, a)]:
            return k
    return None
